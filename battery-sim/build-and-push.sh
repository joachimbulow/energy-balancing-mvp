...
docker build --platform linux/amd64 -t battery .

# 2. Tag the Docker image with the specified version
echo "Tagging Docker image with version $VERSION..."
docker tag battery pemmvpregistry.azurecr.io/battery:v$VERSION
...
docker push pemmvpregistry.azurecr.io/battery:v$VERSION

echo "Done!"
echo "Your image tag name is: pemmvpregistry.azurecr.io/battery:v$VERSION"